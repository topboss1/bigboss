...
def print_input_prompt(prompt, required=True):
...
def get_scan_options():
    ...
    while True:
        ...
        choice = print_menu('스캔 옵션 선택', menu_options)
        ...
        if choice == '1':
            ...
            while True:
                ...
                custom_options = print_input_prompt('원하는 옵션을 입력하세요 (예: -sS -sV -O)')
                ...
                # 옵션 유효성 검사
                is_valid, error_msg = validate_nmap_options(custom_options)
                if not is_valid:
                    print(f'{Fore.RED}{error_msg}')
                    retry = print_menu('다시 시도하시겠습니까?', ['예', '아니오'])
                    if retry == '2':
                        return None
                    continue
